...
package Com::Vmware::Content::Library::Item::StorageStub;

## @class Com::Vmware::Content::Library::Item::Storage
#
#{@name Storage} is a resource that represents a specific instance of a file
#stored on a storage backing. Unlike {@link File}, which is abstract, storage
#represents concrete files on the various storage backings. A file is only
#represented once in {@link File}, but will be represented multiple times
#(once for each storage backing) in {@name Storage}. The {@name
#Storage} {@term service} provides information on the storage backing and the
#specific location of the file in that backing to privileged users who want
#direct access to the file on the storage medium.
#
...
sub new
{
   ...
   my $operations = {
      'get' => {
                ...
                'output_type'=> new Com::Vmware::Vapi::Bindings::Type::ListType(new Com::Vmware::Vapi::Bindings::Type::ReferenceType('module_ctx' => 'Com::Vmware::Content::Library::Item', 'type_name' => 'Storage::Info')),
   ...
   my $self = $class->SUPER::new ('api_provider' => $api_provider,
                                  'iface_name' => 'com.vmware.content.library.item.storage',
