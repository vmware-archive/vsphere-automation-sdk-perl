...
package Com::Vmware::Vcenter::Lcm::LogStub;
...
sub new
{
   ...
   my $get_input_type = new Com::Vmware::Vapi::Bindings::Type::StructType(
      ...
      'fields' => {
                   'task_id' => new Com::Vmware::Vapi::Bindings::Type::StringType(),
      }
   );
   my $get_error_dict = {
   ...
   my $operations = {
      'get' => {
                ...
                'output_type'=> new Com::Vmware::Vapi::Bindings::Type::URIType(),
   ...
   my $self = $class->SUPER::new ('api_provider' => $api_provider,
                                  'iface_name' => 'com.vmware.vcenter.lcm.log',
