...
our %options = (
   ...
   psc => {
      type => "=s",
      variable => "VI_PSC",
      help => "Platform services controller's hostname or IP",
   },
);
...
sub get_option {
   ...
   if (defined($opt->{variable})){
      if (defined($opt->{value}) && $opt->{variable} eq "VI_URL" ){
         my $Url = $opt->{value};
         $Url =~ s|http(s?)://(.*)/sdk.*|http$1://$2/sdk(.*)|i;
         my $protocol = $1;
         ...
         if (defined($protocol)) {
            $opt->{value} = "https://" . $server_from_url . "/sdk";
         }
         else {
            $opt->{value} = "http://" . $server_from_url . "/sdk";
         }
      }
   }
...
sub get_url {
   ...
   if (defined($pscurl)){
      ...
      my $url = $lookupservice->find_vim_url($node_id);
      return $url;
   }
...
sub validate {
   ...
   if (!(option_is_set('help') || option_is_set('version') ||
      ...
      if ((!option_is_set('username') || !option_is_set('password') || 1 == $askPwd)
          && (!option_is_set('sessionfile') )) {
   ...
   if (!option_is_set('ssourl') &&
       option_is_set('ssoauth')) {
      print "The 'ssourl' option must set when 'ssoauth' is enabled";
   }
